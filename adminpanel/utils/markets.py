...
def set_ascendex_market():
    url = 'https://ascendex.com/api/pro/v1/products'
    markets = json.loads(get_url_content(url))['data']
    for market in markets:
        m = market['symbol']
        t = market['baseAsset']
        if len(AscendexMarkets.objects.filter(market=m)) < 1:
            obj = AscendexMarkets(market=m, token=t, tsymbol=t, is_active=True)
            obj.save()
            print(m, t)
...
def set_uniswap_market():
    ...
    umarkets = ModuleUniswap.objects.all()
    for tm in tmarkets:
        # print(tm.contract)
        uObj = ModuleUniswap.objects.filter(tokenid=tm.contract)
        if len(uObj) > 1:
            obj = ModuleUniswap.objects.get(tokenid=tm.contract)[0]
            obj.tsymbol = tm.tsymbol
            obj.is_active = True
        else:
            try:
                obj = ModuleUniswap(tokenid=tm.contract, highest_bid=0, lowest_ask=0, volume=0, tsymbol=tm.tsymbol,
